...
function qs_gen_scripts()
{
	...
	// Enqueue external CSS (Font Awesome & Select2)
	...
	// Enqueue Select2 JavaScript (depends on jQuery)
	...
	wp_enqueue_script('qs-gen-custom', get_template_directory_uri() . '/js/custom.js', array('jquery'), time(), true);
...
function enqueue_load_more_script()
{
	...
	wp_localize_script('load-more-post-js', 'ajax_params', [
		'ajax_url' => admin_url('admin-ajax.php'), // Pass the WordPress admin-ajax.php URL
		'category_id' => get_queried_object_id(), // Pass the current category ID
	]);
}
...
add_action('init', 'handle_post_form_submission');

// function handle_post_form_submission() {
...
//             'post_status'  => 'pending', // You can change this to 'draft' or 'pending'
...
// 		     // Save "Tell Us Your Story" in post meta
// 			 $story = sanitize_textarea_field($_POST['tell_us_your_story']);
// 			 update_post_meta($post_id, '_tell_us_your_story', $story);
...
//        // wp_redirect(add_query_arg('post_created', 'true', get_permalink($post_id)));
// 	   wp_redirect(add_query_arg('post_created', 'true', $_SERVER['REQUEST_URI']));
...
function handle_post_form_submission() {
    if (isset($_POST['action']) && $_POST['action'] === 'create_post') {
        ...
        // Get post status from form
        $post_status = $_POST['post_status'] === 'pending' ? 'pending' : 'draft';
...
function handle_delete_draft_post() {
    if (isset($_POST['post_id']) && isset($_POST['security']) && wp_verify_nonce($_POST['security'], 'delete_post_nonce')) {
        $post_id = intval($_POST['post_id']);
        
        // Check if the post exists and has the desired statuses
        $post = get_post($post_id);
        if ($post && in_array($post->post_status, ['draft', 'pending', 'publish'])) {
            // Delete the post
            wp_delete_post($post_id, true); // Pass 'true' to force delete (permanently)
            wp_send_json_success(); // Return success response
        } else {
            wp_send_json_error(['message' => 'Invalid post status']); // Return error response with a message
        }
    } else {
        wp_send_json_error(['message' => 'Invalid request']); // Return error response for invalid requests
    }
    wp_die(); // Terminate the request properly
}

add_action('wp_ajax_delete_draft_post', 'handle_delete_draft_post');

function handle_post_update_submission() {
    if (isset($_POST['action']) && $_POST['action'] === 'update_post') {
        // Verify nonce
        if (!isset($_POST['update_post_nonce']) || !wp_verify_nonce($_POST['update_post_nonce'], 'update_post_action')) {
            wp_die('Security check failed.');
        }

        // Validate required fields
        if (empty($_POST['post_id']) || empty($_POST['title']) || empty($_POST['description'])) {
            wp_die('Please fill in all required fields.');
        }
        ...
        if (is_wp_error($updated_post_id)) {
            wp_die('Error updating post: ' . $updated_post_id->get_error_message());
        }

        if (isset($_POST['tell_us_your_story'])) {
            $story = sanitize_textarea_field($_POST['tell_us_your_story']);
            update_post_meta($post_id, '_tell_us_your_story', $story);
        }

		 // Remove featured image if the remove_image_flag is set
		 if (!empty($_POST['remove_image_flag']) && $_POST['remove_image_flag'] === '1') {
...
add_action('init', 'handle_post_update_submission');
